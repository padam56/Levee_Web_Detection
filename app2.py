...
def remove_nearby_seepage(sandboil_mask, seepage_mask, distance_threshold):
    """
    Dilate the sandboil mask to create a buffer zone around detected sandboils,
    ...
    # Dilate the sandboil mask to create a buffer zone
    sandboil_dilated = cv2.dilate(sandboil_mask, kernel, iterations=1)
    
    # Remove seepage detections that fall within the dilated sandboil region.
...
def resolve_overlaps(sandboil_mask, seepage_mask, distance_threshold):
    """
    Enhanced overlap resolution that:
      1. Uses constrained flood fill,
      2. Removes smaller overlapping regions,
      3. Prioritizes sandboils over seepage,
      4. And finally removes seepage pixels that are within a specified distance
         from sandboils by using morphological dilation.
    """
